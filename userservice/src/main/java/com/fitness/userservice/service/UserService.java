...
public class UserService {
    ...
    public UserResponse register(RegisterRequest request) {
        log.info("开始注册或同步用户，email={}, keycloakId={}", request.getEmail(), request.getKeycloakId());
    ...
    public UserResponse getUserProfileById(String userId) {
        return toResponse(resolveUserOrThrow(userId));
    }
    ...
    public UserResponse getCurrentUserProfile(String userIdentifier) {
        return toResponse(resolveUserOrThrow(userIdentifier));
    }
    ...
    public User resolveUserOrThrow(String identifier) {
        return resolveUser(identifier)
                .orElseThrow(() -> new UserNotFoundException("未找到用户，identifier=" + identifier));
    }
    ...
    public Optional<User> resolveUser(String identifier) {
        ...
        return userRepository.findById(identifier)
                .or(() -> userRepository.findByKeycloakId(identifier));
    }
    ...
    private UserResponse toResponse(User user) {
        return new UserResponse(
                ...
                user.getGender(),
                user.getGoal(),
                user.getInjuryStatus(),
                user.getFitnessLevel(),
                user.getAssessmentCompleted(),
                user.getAssessmentReport(),
                user.getAssessmentUpdatedAt(),
                user.getCreatedAt(),
                user.getUpdatedAt()
        );
    }
